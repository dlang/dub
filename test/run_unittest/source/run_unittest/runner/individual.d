...
struct TestCaseRunner {
	...
	void run() {
		...
		const neededLocks = testConfig.locks ~ tc;
		runner.acquireLocks(neededLocks, sink);
		scope(exit) {
			runner.releaseLocks(neededLocks, sink);
		}
...
private:
	...
	noreturn failTest(const(char)[] reason, Throwable exception = null) {
