...
Version getDcVerGdmd(string dc) {
	import std.array;
	import std.string;

	immutable progLog = "`" ~ dc ~ " -q,-dumpfullversion --version`";
	immutable p = execute([dc, "-q,-dumpfullversion", "--version"]);
	if (p.status != 0)
		die("Couldn't execute ", progLog);

	immutable parts = p.output.chomp.split('.');
	if (parts.length != 3) {
		log(p.output);
		die(progLog, " didn't output version in 'major.minor.patch' format");
	}

	immutable np = parts.map!(to!int).array;
	return Version(np[0], np[1], np[2], "");
}
