...
*/
void warnOnSpecialCompilerFlags(string[] compiler_flags, Flags!BuildOption options, PackageName name, string config_name)
{
	...
	void outputPreamble()
	{
		...
		got_preamble = true;
		logWarn("");
		if (config_name.empty) logWarn("## Warning for package %s ##", name);
		else logWarn("## Warning for package %s, configuration %s ##", name, config_name);
		logWarn("");
