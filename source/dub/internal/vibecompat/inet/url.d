...
// When using Vibe.d, the below will import the 'regular' `NativePath` & co symbols,
// which will then conflict. Resolve the conflict.
public alias NativePath = dub.internal.vibecompat.inet.path.NativePath;
public alias toNativeString = dub.internal.vibecompat.inet.path.toNativeString;
public alias relativeTo = dub.internal.vibecompat.inet.path.relativeTo;
...
struct URL {
	...
	*/
	...
	URL opBinary(string OP)(InetPath rhs) const if( OP == "~" ) { return URL(m_schema, m_host, m_port, m_path ~ rhs); }
	URL opBinary(string OP)(NativePath rhs) const if( OP == "~" ) { return this.opBinary!OP(cast(InetPath)rhs); }
